/**
 * Skeleton class for implementing sampleables. This class manages the bounding box and leaves it to
 * subclasses to implement the color sampling method. (It might have been reasonable to call this
 * {@code AbstractBoundable}, but I expect it to do more eventually.
 */
public abstract class AbstractSampleable implements Sampleable {
  ...
  @Override
  public Sampleable transform(AffineTransformation trans) {
    Sampleable base = this;
    AffineTransformation inverse = trans.inverse();

    return new AbstractSampleable(trans.apply(getBoundingBox())) {
      @Override
      public Color getColorAt(Point<Double> point) {
        return base.getColorAt(point.transform(inverse));
      }
    };
  }
}
