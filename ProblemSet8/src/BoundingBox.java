...
/**
 * An class for working with bounding boxes. The constructor is private, but there are several
 * public static factory methods for creating bounding boxes. <p> With one notable exception ({@link
 * #EMPTY}), a bounding box is a finite, rectangular area of the plane. We can specify a bounding
 * box by giving the x coordinates of the left and right sides and the y coordinates of the top and
 * bottom sides. <p> Our bounding boxes are considered to include their boundaries.
 */
public final class BoundingBox
    implements Boundable, Interpolable<BoundingBox>, Transformable<BoundingBox> {
  ...
  /**
   * Gets the y coordinate of the top of this bounding box, rounded down to the nearest {@code
   * int}.
   *
  ...
  /**
   * Gets the y coordinate of the bottom of this bounding box, rounded up to the nearest {@code
   * int}.
   *
  ...
  /**
   * Gets the x coordinate of the left side of this bounding box, rounded down to the nearest {@code
   * int}.
   *
  ...
  /**
   * Gets the x coordinate of the right side of this bounding box, rounded up to the nearest {@code
   * int}.
   *
  ...
  /**
   * Determines whether the bounding box of {@code other} is a subset of this bounding box.
   *
  ...
  /**
   * Determines whether the bounding box {@code other} is a subset of this bounding box.
   *
  ...
  /**
   * Joins this bounding box with another bounding box. Returns the smallest bounding box containing
   * both {@code this} and {@code other}.
   *
   * @param other the other bounding box
  ...
  /**
   * Intersects this bounding box with another bounding box. Returns the largest bounding box
   * contained by both {@code this} and {@code other}. If {@code this} and {@code other} are
   * disjoint, returns {@link #EMPTY}.
   *
   * @param other the other bounding box
  ...
  /**
   * Creates a bounding box with the given sides. <p> <strong>Preconditions:</strong> <ul>
   * <li>{@code top <= bottom}</li> <li>{@code left <= right}</li> <li>All four parameters are
   * finite</li> </ul> There is one exception to the preconditions, which is for the empty bounding
   * box {@link #EMPTY}. For that bounding box, the top and left are represented by positive
   * infinity, and the bottom and right by negative infinity. (This naturally results in correct
   * results for a variety of bounding box operations.) This violates <em>all</em> the preconditions
  ...
  /**
   * Returns the largest bounding box contained by all the arguments. If the arguments do not all
   * intersect, returns {@link #EMPTY}.
   *
  ...
  /**
   * Transforms a bounding box according to an affine transformation. Note that this does not
   * transform the box as a rectangle, since bounding boxes are orthogonal to the axes. Rather, it
   * finds the minimal bounding box for this bounding box transformed as a rectangle, which may be
   * larger than this bounding box.
   *
   * @param trans the transformation to apply
   * @return the transformed bounding box
   */
  @Override
  public BoundingBox transform(AffineTransformation trans) {
    double x1 = trans.applyX(left, top);
    double x2 = trans.applyX(left, bottom);
    double x3 = trans.applyX(right, top);
    double x4 = trans.applyX(right, bottom);

    double y1 = trans.applyY(left, top);
    double y2 = trans.applyY(left, bottom);
    double y3 = trans.applyY(right, top);
    double y4 = trans.applyY(right, bottom);

    return fromTBLR(min(y1, min(y2, min(y3, y4))),
