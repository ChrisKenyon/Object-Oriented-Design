...
/**
 * Perhaps the simplest possible implementation of the {@link Raster} interface, this class
 * represents a raster as an array list of array lists of integers.
 */
public final class ListRaster extends AbstractRaster {
   ...
   *
   * <p><strong>Precondition:</strong> {@code array.length >= width() * height()}
   *
  ...
  /**
   * Allocates and fills a new {@link ArrayList}, using a function object to generate the elements.
   * That is, each index {@code i} of the array list will map to the value {@code f.apply(i)}.
   *
   * @param size the size of the new {@code ArrayList}
   * @param f    the function object mapping array indices to initial values
   * @param <E>  the element type
   * @return a new {@code ArrayList} with elements from {@code f.apply(0)} up to {@code f.apply(size
   * - 1)}.
   */
  public static <E>
